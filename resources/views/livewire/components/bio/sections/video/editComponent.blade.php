<?php
   ...
   state([
       'types' => [
            'youtube' => [
                'name' => 'Youtube',
                'image' => gs('assets/image/logos/youtube.png'),
                'isIframe' => false
            ],
            'vimeo' => [
                'name' => 'Vimeo',
                'image' => gs('assets/image/logos/vimeo.svg'),
                'isIframe' => false
            ],
            'twitch' => [
                'name' => 'Twitch',
                'image' => gs('assets/image/logos/twitch.png'),
                'isIframe' => true
            ],
            'soundcloud' => [
                'name' => 'SoundCloud',
                'image' => gs('assets/image/logos/soundcloud.png'),
                'isIframe' => true
            ],
            'dailymotion' => [
                'name' => 'Daily Motion',
                'image' => gs('assets/image/logos/dailymotion.svg'),
                'isIframe' => false
            ],
       ]
   ]);

   $fetch = function($link, $type){
        if(empty($link)) return;

        $isIframe = false;

        if(ao($this->types, "$type.isIframe")) $isIframe = true;
        ...
        $data = [
            'type' => $type,
            'link' => $link,
            'thumbnail' => getPlatformThumbnail($type, $link),
            'isIframe' => $isIframe
        ];

        $data['iframe'] = getEmbedableLink($type, $link);

        return $data;
   };
?>
<div >
    <div x-data="builder__sectionVideo">
            ...
            <div class="website-section !block">
                <div class="design-navbar">
                    <ul >
                       ...
                       <li class="!pl-0" x-text="!section.content.title ? '{{ __('Video') }}' : section.content.title"></li>
                    ...
                    <div class="container-small tab-content-box mt-0 ![overflow:initial] lg:!overflow-y-auto">
                        <div class="tab-content">
                            <div x-cloak x-show="__tab == 'content'" data-tab-content>
                                <x-livewire::components.bio.sections.video.partial.edit.content/>
                            </div>
                        </div>
                    </div>
                </div>
  ...
  @script
  <script>
      Alpine.data('builder__sectionVideo', () => {
         return {
            autoSaveTimer: null,
