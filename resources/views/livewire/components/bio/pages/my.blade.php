...
<?php
   ...
   $redirectPage = function($id){
      $this->skipRender();
      if(!$page = BioSite::where('user_id', iam()->id)->where('id', $id)->first()) return;
      $this->site->user->_last_edited_bio = $page->id;
      $this->site->user->save();

      return redirect(route('dashboard-bio-index', ['slug' => $page->_slug]));
   };
?>
                     ...
                     <div class="mt-2">
                        <div class="flex flex-col gap-4">
                           ...
                           <div class="border-gray-50 relative rounded-lg border-2 bg-white p-3 pr-1 shadow-lg transition-all hover:shadow-md sm:p-4">
                              ...
                              <a x-data="{confirm:false}" @click="confirm=true; $wire.redirectPage('{{ $item->id }}')" class="yena-black-btn mt-1 !justify-between cursor-pointer">{!! __i('--ie', 'settings.8') !!}
                                 <div class="flex items-center">
                                    {{ __('Manage') }} 
