<div class="w-[100%] section-width-fill section-bg-wrapper list-box" :class="{
   ...
   'lr-padding': section.section_settings.width == 'fit',
    ...
    @endphp
      <section class="list-content section-content wire-section" x-intersect="__section_loaded($el)" x-data="builder__productSectionView" :class="{
               ...
               <div class="section-container grid [--scroll-speed:22.5s]" :class="{
                  ...
                  <template x-for="(item, index) in window._.sortBy(section.items, 'position')" :key="item.uuid">
                     <div x-data="{product: getProduct(item.content.product_id)}">
                        <a class="product-card-container !border-0" x-outlink:navigate="product.route" :class="{
