...
use App\Models\Base\MediakitSite as BaseMediakitSite;
use Illuminate\Database\Eloquent\Casts\Attribute;

class MediakitSite extends BaseMediakitSite
{
	protected $fillable = [
		'user_id',
		'name',
    ...
    public function deleteCompletely(){
        ...
        MediakitSitesLinkerTrack::where('site_id', $this->id)->delete();
        MediakitSitesVisitor::where('site_id', $this->id)->delete();
        MediakitSiteDomain::where('site_id', $this->id)->delete();
        MediakitSiteSocial::where('site_id', $this->id)->delete();

        $uploads = MediakitSitesUpload::where('site_id', $this->id)->get();
    ...
    public function duplicateSite($surfix = '_copy'){
        ...
        foreach($pages as $_page){
            ...
            $layouts = MediakitSection::where('page_id', $_page->uuid)->get();
	...
	{
		return $this->hasMany(MediakitSiteSocial::class, 'site_id', 'id');
	}

	public function getLogo(){
        $avatar = $this->logo;
        $default = "https://api.dicebear.com/6.x/bottts-neutral/svg?seed=$this->name";
        $check = mediaExists('media/mediakit/images', $avatar);
        $path = getStorage('media/mediakit/images', $avatar);
	...
	{
		return $this->hasMany(MediakitSitesUpload::class, 'site_id', 'id');
	}
	...
	{
		return $this->hasMany(MediakitSection::class, 'site_id', 'id');
	}

	public function getInsight(){
        // Get Model
        $visitors = MediakitSitesVisitor::where('site_id', $this->id)->get();
        ...
        $visitors_this_year = MediakitSitesVisitor::where('site_id', $this->id)->where('created_at', '>=', $start_of_year)->get();
