        ...
        <div class="panel-input mb-1 px-[var(--s-2)]">
           <form>
              ...
              <div class="accordion mt-4">
               <div x-ref="sortable_wrapper" class="grid grid-cols-1 md:grid-cols-2 gap-2">
                  <template x-for="(item, index) in window._.sortBy(section.items, 'position')" :key="item.uuid" x-ref="sortable_template">
                     <div>
                        <x-livewire::components.bio.sections.twitter.partial.edit.--item />
                     </div>
                  </template>
               </div>
               
               <div class="mt-4 accordion-item add-new-accordion !rounded-full" @click="createItem">
                  <button class="accordion-header !px-4" type="button">
                     <p ><span >{{ __('Add Twitter') }}</span></p>
