...
<?php
   ...
   <div class="p-5 w-full mt-1">
      <div class="--placeholder-skeleton w-full h-[100px] rounded-[var(--yena-radii-sm)]"></div>
      <div class="--placeholder-skeleton w-full h-[100px] rounded-[var(--yena-radii-sm)] mt-1"></div>
      <div class="--placeholder-skeleton w-full h-[100px] rounded-[var(--yena-radii-sm)] mt-1"></div>
   </div>');
   ...
   mount(function(){
      if(__o_feature('feature.insight')){
         ...
         $this->live = MySession::activity(10)->where('site_id', $this->site->id)->limit(5)->get();
         $this->linksVisit = (new SitesLinkerTrack)->totalVisits($this->site);
      }
   });
?>
...
<div>
   <div x-data="builder__page_insight">
      {{-- <div x-show="__page == 'views'">
         <div>
            <livewire:components.builder.parts.insight.views lazy :$site :key="uukey('builder', 'insight-page-views')">
         </div>
     </div> --}}
    ...
    <div x-show="__page == 'clicks'">
      <div>
         <livewire:components.builder.parts.insight.clicks lazy :$site :$linksVisit :key="uukey('builder', 'insight-page-clicks')">
      </div>
    </div>
    <div x-show="__page == 'live'">
      <div>
         <livewire:components.builder.parts.insight.live lazy :$site :$linksVisit :key="uukey('builder', 'insight-page-live')">
      </div>
    </div>
        ...
        <div class="px-[var(--s-2)] py-[0] mt-[var(--s-2)] {{ !__o_feature('feature.insight') ? 'hidden' : '' }}">
           <div class="details-item w-full p-3 md:p-5 z[box-shadow:0_4px_6px_-1px_rgba(0,_0,_0,_0.1),0_2px_4px_-1px_rgba(0,_0,_0,_0.06)] mb-2 rounded-md !bg-white [border:1px_solid_var(--c-mix-1)]">
               <div class="details-head">
                   ...
                   <div class="menu--icon !bg-black !text-[10px] !text-white !w-auto !px-2 !h-5 !cursor-pointer !flex !items-center !justify-center !ml-auto !gap-1 !hidden" @click="_load_page('insight.views')">
           ...
           <div class="details-item w-full p-3 md:p-5 z[box-shadow:0_4px_6px_-1px_rgba(0,_0,_0,_0.1),0_2px_4px_-1px_rgba(0,_0,_0,_0.06)] mb-2 rounded-md !bg-white [border:1px_solid_var(--c-mix-1)]">
              <div class="details-head">
                  ...
                  <div class="menu--icon !bg-black !text-[10px] !text-white !w-auto !px-2 !h-5 !cursor-pointer !flex !items-center !justify-center !ml-auto !gap-1 !hidden" @click="__page='live'">
                      <i class="fi fi-rr-eye"></i>
                      {{ __('View') }}
                  </div>
              </div>
              <div class="details-counter text-2xl md:text-4xl truncate font-bold truncate">{{ \App\Models\MySession::activity(10)->where('site_id', $site->id)->count() }}</div>
