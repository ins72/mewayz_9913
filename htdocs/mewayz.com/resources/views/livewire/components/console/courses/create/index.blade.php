...
<?php
   ...
   $createCourse = function(){
      ...
      if(!empty($this->image)){
         ...
         $filesystem = sandy_filesystem('media/courses/image');
         $image = $this->image->storePublicly('media/courses/image', $filesystem);
         $image = str_replace("media/courses/image/", "", $image);
         $a->banner = $image;
      }
      ...
      $this->flashToast('success', __('Course created'));
      $this->dispatch('close');

      $this->dispatch('coursesRefresh');
   };
?>
...
<div class="w-full">
   <div class="flex flex-col">
      ...
      <header class="flex-[0_1_0%] py-4 text-3xl px-6 font-extrabold tracking-[-1px]">{{ __('Create Courses') }}</header>

      <hr class="yena-divider">

      <form wire:submit="createCourse" class="px-8 pt-2 pb-6">
