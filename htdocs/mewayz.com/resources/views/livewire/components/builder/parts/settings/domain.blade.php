...
<?php
    ...
    $getDomain = function(){
        $this->domain = SiteDomain::where('site_id', $this->site->id)->first();
    ...
    $deleteDomain = function(){
        ...
        if(config('app.aapanel_enable')){
            try {
                $api = new \App\Yena\aaPanel();
                $api = $api->RemoveDomain($this->domain->host);
            } catch (\Throwable $th) {
                //throw $th;
            }
        }
    ...
    $createDomain = function($host = null, $protocol = null){
        ...
        if(SiteDomain::where('site_id', $this->site->id)->where('host', parse($valid_url, 'host'))->first()){
            return [
                'status' => 'error',
        ...
        if(config('app.aapanel_enable')){
            try {
                $api = new \App\Yena\aaPanel();
                $api = $api->AddDomain(parse($valid_url, 'host'));
            } catch (\Throwable $th) {
                //throw $th;
            }
        }
