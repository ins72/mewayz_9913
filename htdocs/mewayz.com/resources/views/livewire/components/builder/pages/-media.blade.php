...
updated([
    'image' => function(){
        ...
        foreach ($this->image as $photo) {
            $image = $photo->storePublicly('media/site/images', $filesystem);
...
$getMedia = function(){
   $this->diskSizePercent = get_used_storage_percent($this->site);
    $this->medias = SitesUpload::where('site_id', $this->site->id)->where('is_ai', 0)->where('trashed', 0)->get()->map(function($item){
...
$deleteMedias = function(){
   foreach ($this->selectedMedias as $item) {
      if(!$media = SitesUpload::where('site_id', $this->site->id)->where('id', $item)->first()) continue;
      $media->trashed = 1;
      $media->save();

      storageDelete('media/site/images', $media->path);
...
$getAiMedia = function(){
    $this->aiMedia = SitesUpload::where('site_id', $this->site->id)->where('is_ai', 1)->where('trashed', 0)->get()->map(function($item){
...
<div x-data="media__section">
    <div class="media-section">
        ...
        <div class="container-small sticky">
            <div class="tab-link">
                <ul class="tabs">
                  <li class="tab !w-full !mr-0" @click="__tab = 'uploads'" :class="{'active': __tab == 'uploads'}">{{ __('Uploads') }}</li>

                  <li class="tab !w-full !mr-0" @click="__tab = 'unsplash'; getUnsplashMedia()" :class="{'active': __tab == 'unsplash', '!hidden': multiSelect}">{{ __('Unsplash') }}</li>

                  <li class="tab !w-full" @click="__tab = 'ai'" :class="{'active': __tab == 'ai', '!hidden': multiSelect}">
