...
updated([
    'image' => function(){
        ...
        $filesystem = sandy_filesystem('media/bio/images');
        $images = [];
        
        foreach ($this->image as $photo) {
            $image = $photo->storePublicly('media/bio/images', $filesystem);
            $image = str_replace("media/bio/images/", "", $image);
            $size = storageFileSize('media/bio/images', $image);
            $upload = new BioSitesUpload;
...
$getMedia = function(){
   $this->diskSizePercent = get_used_storage_percent($this->site);
    $this->medias = BioSitesUpload::where('site_id', $this->site->id)->where('is_ai', 0)->where('trashed', 0)->get()->map(function($item){
...
$deleteMedias = function(){
   foreach ($this->selectedMedias as $item) {
      if(!$media = BioSitesUpload::where('site_id', $this->site->id)->where('id', $item)->first()) continue;
      $media->trashed = 1;
      $media->save();

      storageDelete('media/bio/images', $media->path);
...
$getAiMedia = function(){
    $this->aiMedia = BioSitesUpload::where('site_id', $this->site->id)->where('is_ai', 1)->where('trashed', 0)->get()->map(function($item){
...
$saveAiMedia = function($image, $url, $upload_id){
   ...
   \Storage::disk($filesystem)->put("media/bio/images/$image", file_get_contents($url));
   $size = storageFileSize('media/bio/images', $image);

   $upload = BioSitesUpload::find($upload_id);
   $upload->size = $size;
   $upload->name = basename($image);
   $upload->path = $image;
