...
<?php
   ...
   on([
      ...
      'section::deleteItem' => function ($item) {
         if(!$_item = BioSectionItem::where('uuid', $item)) return;
      ...
      'builder::save_sections_and_items' => function ($section, $js = null){
         ...
         if(is_array($items = __a($section, 'items'))){
            foreach ($items as $key => $value) {
               if(!$_item = BioSectionItem::where('uuid', __a($value, 'uuid'))->first()) continue;
               $_item->fill($value);
      ...
      'builder::saveSite' => function(){
         ...
         if(is_array($socials = __a($this->siteArray, 'socials'))){
            foreach ($socials as $key => $value) {
               if(!$_social = BioSiteSocial::where('uuid', __a($value, 'uuid'))->first()) continue;
               $_social->fill($value);
   ...
   $deleteSection = function($id){
      ...
      if(!$section = BioSection::where('uuid', $id)->where('site_id', $this->site->id)->first()) return;

      BioSectionItem::where('section_id', $section->uuid)->delete();
...
<div>
   <div class="editor-panel bio-builder-wrapper" x-data="yenaBuilder" :class="{'maximize': renderView == 'max'}">
      ...
      <livewire:components.bio.layout.toolbar :$site :key="uukey('builder', 'toolbar')" />
      <livewire:components.bio.parts.openai :key="uukey('builder', 'openai-backend')" />
      {{-- <livewire:components.bio.parts.ai :key="uukey('builder', 'ai-text-backend')" /> --}}
      
      <div class="container !max-w-[initial]">
         <div class="projects">
            <livewire:components.bio.layout.sidebar :$site lazy="on-load" :key="uukey('builder', 'sidebar')" />
            ...
            <div class="container-small !max-w-[initial] edit-board">
               <div class="flex flex-col w-[100%]">
                  <div class="edit-blocks-container flex-1">
                     @persist('builder')
                        <div wire:ignore class="lg:flex">
                           <livewire:components.bio.build lazy :$site :key="uukey('builder', 'components.bio.build')" />
                                    ...
                                    <div class="flex flex-col items-center px-6 pb-0">
                                       <div class="PreviewM PagePreviewWrapper" @click="$event.stopPropagation()" :class="{
                                          ...
                                          <div class="PagePreview">
                                             <div class="h-full bg-white remove-scrollbar overflow-y-auto overflow-x-hidden" {{-- x-portal="#mobile-preview" x-portal:screen="-640" x-portal:is="true" --}}>
                                                <div wire:ignore>
                                                   <livewire:components.bio.index lazy :$site :key="uukey('builder', 'components.bio.index')" />
                                                </div>
                                             </div>
                                          </div>
                                       </div>
      ...
      <template x-teleport="body">
         <div class="overlay backdrop delete-overlay" :class="{
             ...
             <div class="delete-site-card !border-0 !rounded-2xl !shadow-lg md:!w-[calc(var(--unit)*_60)] bg-[#fff] h-[90%]" @click="$event.stopPropagation()">
                <div class="overlay-card-body !rounded-2xl overflow-auto h-full p-5">
                    
                  <livewire:booking.single :user="$site->user" :key="uukey('app', 'components.booking.singleComp')"/>
                </div>
             </div>
          </div>
     </template>
            ...
            @foreach($s as $key => $item)
               ...
               @if ($n = __a($item, 'components.alpinePost'))
                   <div class="section-post-{{ $key }}">
                     <livewire:is :component="'components/bio/sections/' . $key . '/' . basename($n)" :key="uukey('p::component', 'component:post-section' . $n)">
                   </div>
               @endif
            @endforeach
         @endif
  ...
  <template x-ref="translate_template">
   ...
   <template x-teleport="body">
      <x-modal name="share-modal" :show="false" removeoverflow="true" maxWidth="2xl" focusable>
         ...
         <livewire:components.bio.parts.share :key="uukey('builder', 'share-modal')">
      </x-modal>
   </template>
   @script
        <script>
            Alpine.data('yenaBuilder', () => {
               return {
                  ...
                  navigatePage(page){
                     if(page == '__last_state' && this.__last_page) {
