...
<?php
    ...
    $getPixels = function(){
        $pixels = SitePixel::where('site_id', $this->site->id)->get();
    ...
    $deletePixel = function($uuid){
        ...
        if(!$pixel = SitePixel::where('uuid', $uuid)->where('site_id', $this->site->id)->first()) return;
    ...
    $editPixel = function ($item){
        ...
        if(!$pixel = SitePixel::where('uuid', ao($item, 'uuid'))->where('site_id', $this->site->id)->first()) return;
