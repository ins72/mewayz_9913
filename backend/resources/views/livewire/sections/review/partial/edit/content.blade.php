    ...
    <div class="mt-2 content">
        <div class="panel-input mb-1 px-[var(--s-2)]">
           <form>
              ...
              <div class="input-box">
               <label for="text-size">{{ __('Type') }}</label>
               <div class="input-group align-type">
                  <button class="btn-nav !w-[50%] active" type="button" @click="section.settings.type = 'quote'" :class="{'active': section.settings.type =='quote'}">
                     {!! __i('--ie', 'quote') !!}
                  </button>
                  <button class="btn-nav !w-[50%]" type="button" @click="section.settings.type = 'stars'" :class="{'active': section.settings.type == 'stars'}">
                     {!! __i('--ie', 'star-favorite') !!}
                  </button>
               </div>
            </div>
              ...
              <div class="accordion">
               <div x-ref="sortable_wrapper" class="flex flex-col gap-2">
                  <template x-for="(item, index) in window._.sortBy(section.items, 'position')" :key="item.uuid" x-ref="sortable_template">
                     ...
                     <button class="panel-button media-button !justify-start" @click="__page = 'section::review::' + item.uuid" type="button">
                        <div class="handle">
