...
render(function (View $view, $ids) {
    if(!$site = Site::where('address', $ids[0])->first()) abort(404);
	...
	if (empty($ids[1])) {
		...
		if (!$page) {
			$page = $site->pages()->first(); // Get the first page if no default found
		}
		if ($page) {
			$seo = site_seo($page);
		}
    ...
    if(__o_feature('consume.site_traffic', $site->user) != -1){
        ...
        $month_visitors = cache()->remember($key, 60 * 24, function () use($site) {
            return SitesVisitor::whereMonth('created_at', \Carbon\Carbon::now()->month)->where('site_id', $site->id)->count();
        });
    ...
    // Track Visits
    if ($canVisit) {
        if ($vistor = SitesVisitor::where('session', Session::getId())->where('site_id', $site->id)->first()) {
            $vistor->views = ($vistor->views + 1);
            $vistor->save();
        }else{
            $new = new SitesVisitor;
            $new->site_id = $site->id;
    ...
    if(!$check = \App\Models\MySession::where('id', \Session::getId())->where('site_id', $site->id)->first()){
