...
use Session;
use App\Models\Base\SitesLinkerTrack as BaseSitesLinkerTrack;

class SitesLinkerTrack extends BaseSitesLinkerTrack
{
	protected $fillable = [
		'linker',
		'site_id',
	...
	public function getLinkInsight($slug, $user){
        // Get Model
        $visitors = SitesLinkerTrack::where('slug', $slug)->where('site_id', $user->id)->get();
    ...
    public function track($link, $site_id){
        ...
        // Track Visits
        if ($vistor = SitesLinkerTrack::where('session', Session::getId())->where('slug', $linker->slug)->first()) {
            $vistor = SitesLinkerTrack::find($vistor->id);
            $vistor->views = ($vistor->views + 1);
            $vistor->save();
        }else{
            $new = new SitesLinkerTrack;
            $new->site_id = $site_id;
