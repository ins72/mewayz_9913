...
<form class="items-center" method="GET">
  ...
  <div class="grid grid-cols-2 gap-4 overflow-y-auto">
    ...
    <div class="form-input ">
      <label class="initial mb-0">{{ __('Plan') }}</label>
      <select name="plan">
        <option value="">{{ __('All') }}</option>
    ...
    <button type="submit" class=" bg-primary focus:bg-primary-400 hover:bg-primary-400 block appearance-none rounded-md text-sm font-medium text-white duration-100 focus:outline-none disabled:opacity-75 px-4 py-2.5 w-full mt-4">
      <div class="relative flex items-center justify-center ">
         <div class="duration-100">{{ __('Submit') }}</div>
      </div>
   </button>
   <a class="text-blue-600 mt-4 font-bold block" href="">{{ __('Reset all filters') }}</a>
</form>