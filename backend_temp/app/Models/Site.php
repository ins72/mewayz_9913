...
class Site extends BaseSite
{
    ...
    public function deleteCompletely(){
        ...
        SitesLinkerTrack::where('site_id', $this->id)->delete();
        SitesVisitor::where('site_id', $this->id)->delete();
        SiteDomain::where('site_id', $this->id)->delete();
        SiteForm::where('site_id', $this->id)->delete();
        SiteSocial::where('site_id', $this->id)->delete();
        SitesStaticThumbnail::where('site_id', $this->id)->delete();

        $uploads = SitesUpload::where('site_id', $this->id)->get();
    ...
    public function duplicateSite($surfix = '_copy'){
        ...
        foreach($pages as $_page){
            ...
            $layouts = Section::where('page_id', $_page->uuid)->get();
	...
	{
		return $this->hasMany(SiteSocial::class, 'site_id', 'id');
	}

	public function getLogo(){
        $avatar = $this->logo;
        $default = "https://api.dicebear.com/6.x/bottts-neutral/svg?seed=$this->name";
        $check = mediaExists('media/page/logo', $avatar);
        $path = getStorage('media/page/logo', $avatar);
	...
	{
		return $this->hasMany(SitesUpload::class, 'site_id', 'id');
	}
	...
	/**
	 * Get all of the posts for the Site
	 *
	 * @return \Illuminate\Database\Eloquent\Relations\HasMany
	 */
	...
	{
		return $this->hasMany(Section::class, 'site_id', 'id');
	}
	...
	public function getInsight(){
        // Get Model
        $visitors = SitesVisitor::where('site_id', $this->id)->get();
        ...
        $visitors_this_year = SitesVisitor::where('site_id', $this->id)->where('created_at', '>=', $start_of_year)->get();
