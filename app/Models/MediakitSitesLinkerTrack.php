...
use App\Models\Base\MediakitSitesLinkerTrack as BaseMediakitSitesLinkerTrack;

class MediakitSitesLinkerTrack extends BaseMediakitSitesLinkerTrack
{
	protected $fillable = [
		'linker',
		'site_id',
    ...
    public function topLink($user, $limit = 5){
    	$model = new MediakitSitesLinkerTrack;
	...
	public function getLinkInsight($slug, $user){
        // Get Model
        $visitors = MediakitSitesLinkerTrack::where('slug', $slug)->where('site_id', $user->id)->get();
    ...
    public function track($link, $site_id){
        ...
        // Track Visits
        if ($vistor = MediakitSitesLinkerTrack::where('session', Session::getId())->where('slug', $linker->slug)->first()) {
            $vistor = MediakitSitesLinkerTrack::find($vistor->id);
            $vistor->views = ($vistor->views + 1);
            $vistor->save();
        }else{
            $new = new MediakitSitesLinkerTrack;
            $new->site_id = $site_id;
